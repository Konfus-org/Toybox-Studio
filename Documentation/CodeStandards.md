...
- **One public class per `.cs` file**; one View per `.axaml` file. Small private helper types tightly owned by the file may share it, but prefer their own file.
...
- **Do not use regions** If a region is needed its a sign we need to break up the class.
