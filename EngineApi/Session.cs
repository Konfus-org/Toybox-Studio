...
/// to an already-running instance without taking it over. Drives the shared <see cref="Engine"/>'s
...
public sealed class Session : IAsyncDisposable
{
        ...
        Engine engine)
    {
        ...
        // and a dropped connection (or an owned-process exit) funnels into the same loss handler.
    ...
    public async Task StopAsync()
    {
        ...
        try
        {
            ...
            if (_engine.IsConnected && _engine.IsRunning)
            {
                ...
                await _engine.SendCommand(EngineMethods.EngineShutdown, null, cts.Token).ContinueOnAnyContext();

                if (!await _engine.WaitForExitAsync(GracefulExitTimeout).ContinueOnAnyContext())
    ...
    public async Task SetPausedAsync(bool isPaused)
    {
        ...
        var result = await _engine
            .SendCommand(EngineMethods.EngineSetPaused, new { IsPaused = isPaused }, cts.Token)
            .ContinueOnAnyContext();
        if (!result.Success)
        {
    ...
    /// <summary>
    /// Starts the current project's engine (compile + launch). Routed through the transition gate so a manual
    /// start can't interleave with an auto-attach or a crash auto-restart; a no-op while a session is already
    /// starting or live. Used by the menu's "Engine ▸ Start" action and the viewport's launch button.
    /// </summary>
    public Task StartAsync() => LaunchSerializedAsync();

    /// <summary>
    /// Restarts the engine: tears down whatever is running (an owned process is killed, an attached one merely
    /// detached) and relaunches a freshly built owned engine. Held under the transition gate so the stop→launch
    /// can't be interleaved by another transition. From a disconnected state this is just a start.
    /// </summary>
    public Task RestartAsync() => RestartForProjectAsync();
    ...
    private void CompleteConnection(Hello hello)
    {
        ...
        _log.SetEngineForwarder((level, message) =>
            _engine.SendNotification(EngineMethods.EditorLog, new { Level = level, Message = message }));
        _log.SetLogColorSink((info, warning, error, ct) =>
            _engine.SendCommand(
                EngineMethods.EngineSetLogColors, new { Info = info, Warning = warning, Error = error }, ct));
        _log.Info(
    ...
    private async Task RunWatchdogAsync(CancellationToken ct)
    {
        try
        {
            while (!ct.IsCancellationRequested)
            {
                ...
                var ping = await _engine
                    .SendCommand(EngineMethods.EnginePing, null, attempt.Token).ContinueOnAnyContext();
                ...
                {
                    Interlocked.Exchange(ref _lastPingOkTicks, DateTime.UtcNow.Ticks);
                    PingMeasured?.Invoke(stopwatch.Elapsed);
                    MarkResponsive();
    ...
    private async Task TearDownAsync(bool killProcess)
    {
        ...
        lock (_sync)
        {
            if (State == ConnectionState.Disconnected && !_engine.IsRunning && !_engine.IsConnected)
                return;
        ...
        // engine's Disconnected event (fired by Disconnect below) must not be mistaken for a crash. The
        // owned process's Exited (fired by the kill below) is guarded by the same flag.
        ...
        if (killProcess && _engine.IsRunning)
            _log.Warning("Killing engine process.");
        // Release the owned process handle: when killing, wait for it to actually exit — the engine binaries
        // are built in-tree, so a lingering handle would make the next compile's relink fail with a
        // permission-denied error. A no-op for an attached session (there is no owned process).
        await _engine.StopProcessAsync(killProcess, GracefulExitTimeout).ContinueOnAnyContext();
    ...
    private async Task SetPlayingAsync(bool isPlaying)
    {
        ...
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(2));
        var result = await _engine
            .SendCommand(EngineMethods.EngineSetPlaying, new { IsPlaying = isPlaying }, cts.Token)
            .ContinueOnAnyContext();
        if (!result.Success)
        {
