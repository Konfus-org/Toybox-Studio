...
/// </summary>
public sealed partial class EntityViewModel : ObservableObject, IWorldMenuTarget
{
    ...
    // The latest snapshot for this id — reassigned each reconcile (UpdateFrom). It serves both as the data
    // source and as the handle the component RPCs run through.
    ...
    public EntityViewModel(Entity entity, Func<Task> resync)
    {
        _entity = entity;
        _resync = resync;
    }
    ...
    // A tree row is a world menu target: it's an entity, opened from the tree surface (so rename/reorder show).
    ulong? IWorldMenuTarget.EntityId => Id;
    ...
    public void UpdateFrom(Entity data)
    {
        ...
        // The components are already typed handles carrying their own Raw; the grids read that Raw directly
        // (JsonDescriptorReader) or via reflection.
        ...
        foreach (var component in ordered)
        {
            ...
            if (component.Name == ScriptComponentName)
                scripts = new ScriptContainerViewModel(component, _resync);
            else
                Components.Add(new ComponentViewModel(component, _resync));
        }
    ...
    private async Task ApplyJsonAsync()
    {
        ...
        foreach (var component in parsed.Properties())
        {
            ...
            var result = await _entity.GetComponent(component.Name)
                .Deserialize(value, CancellationToken.None)
                .ContinueOnSameContext();
            if (!result.Success)
            {
