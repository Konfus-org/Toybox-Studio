...
public sealed partial class BillboardViewModel(ulong id, string name, IReadOnlyList<BillboardIcon> icons)
    : ObservableObject, IWorldMenuTarget
{
    public ulong Id { get; } = id;

    // A billboard is a world menu target: it's an entity, opened from the viewport (so rename/reorder hide).
    ulong? IWorldMenuTarget.EntityId => Id;
    WorldMenuSurface IWorldMenuTarget.Surface => WorldMenuSurface.Viewport;
