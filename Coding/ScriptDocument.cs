...
/// <summary>
/// One open source file as a single shared buffer. Every editor surface asking for the same file binds to the
/// same instance (vended by <see cref="ScriptService"/> keyed by path), so the unsaved text, dirty state, and
/// title stay consistent no matter which surface the user typed in. Dirtiness is derived by comparing the live
/// <see cref="Text"/> against the last text written to disk.
/// </summary>
