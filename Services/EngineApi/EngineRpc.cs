...
public sealed class EngineRpc : IAsyncDisposable
{
    ...
    /// <summary>
    /// Fetches a material asset's reflected fields by id (the enriched per-field shape entity.describe
    /// emits), so the inspector can show a MaterialInstance's base parameter/texture values — the slots its
    /// overrides apply on top of. Replies <c>{ material }</c>.
    /// </summary>
    public Task<Result<JObject>> DescribeAssetAsync(long assetId, CancellationToken ct) =>
        InvokeAsync<JObject>("asset.describe", new { AssetId = assetId }, ct);
    ...
    /// The project's own lean AppSettings.json is merged over these defaults for editing, so the settings grid
    /// can show every graphics/physics/etc. setting and offer add/remove on the plugins list. Returns the
    /// inner settings field-map (the engine replies <c>{ settings }</c>).
    /// </summary>
    public async Task<Result<JObject>> DescribeSettingsAsync(CancellationToken ct)
    {
        var result = await InvokeAsync<JObject>("app.describeSettings", null, ct).ContinueOnAnyContext();
    ...
    /// <summary>Renames an entity in place (the world list's inline rename).</summary>
    public Task<Result> SetEntityNameAsync(ulong entityId, string name, CancellationToken ct) =>
        InvokeAsync("entity.setName", new { EntityId = entityId, Name = name }, ct);
