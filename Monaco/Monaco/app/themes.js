...
(function () {
  ...
  // `primary` (an "RRGGBB" hex, no "#", or null) is the app theme's primary colour: when given, the editor's
  // accents — caret, selection, focus border, active line number — are recoloured to it so Monaco picks up the
  // Studio theme's primary. Null leaves the base theme's own accent defaults.
  function define(monaco, name, base, p, primary) {
    var colors = {
    ...
    if (primary) {
      var hex = "#" + primary;
      colors["editorCursor.foreground"] = hex;
      colors["editor.selectionBackground"] = hex + "55";          // translucent so text stays legible
      colors["editor.inactiveSelectionBackground"] = hex + "33";
      colors["editor.selectionHighlightBackground"] = hex + "22";
      colors["editorLineNumber.activeForeground"] = hex;
      colors["focusBorder"] = hex;
    }
  ...
  window.__tbxThemes = {
    dark: "toybox-dark",
    light: "toybox-light",
    // Called from editor.js after editor.main loads (no primary), and again on each "theme" envelope with the
    // app theme's primary so the accents track it. Re-defining a live theme needs a setTheme to take — the
    // caller does that right after.
    register: function (monaco, primary) {
      define(monaco, "toybox-dark", "vs-dark", DARK, primary);
      define(monaco, "toybox-light", "vs", LIGHT, primary);
    },
  };
})();
