...
public sealed class CommandRunner
{
    ...
    /// <summary>
    /// Runs <paramref name="fileName"/> with <paramref name="arguments"/> to completion. The result's value
    ...
    {
        using var process = Build(fileName, arguments, workingDirectory, logOutput, logCategoryRegex);
        if (!TryStart(process, fileName, arguments, logOutput, out var failure))
            return failure;
        ...
        catch (OperationCanceledException)
        {
            ...
            return TimedOut(fileName, timeout!.Value);
        }

        process.WaitForExit(); // Drain the async output handlers before reporting.
        return FromExit(fileName, process.ExitCode);
    }
