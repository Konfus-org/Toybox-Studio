...
/// and reads/writes a project's settings (the engine's AppSettings.json). Missing or invalid editor
/// settings yield defaults.
/// </summary>
public sealed class Settings
{
    ...
    /// <summary>
    /// Reads the project's full settings document as a JObject, or null. The backing file is the
    /// engine's AppSettings.json (asset type tbx::AppSettings); "project settings" is the editor-side name.
    /// </summary>
    public JObject? ReadProjectSettingsJson(ProjectInfo? project)
    {
        if (project is null)
            return null;
    ...
    /// <summary>
    /// Writes a full project settings document back to the project's AppSettings.json.
    /// </summary>
    public bool TrySaveProjectSettingsJson(ProjectInfo? project, JObject json, out string? error)
    {
        error = null;
        if (project is null)
        {
        ...
        {
            error = $"Failed to save project settings: {exception.Message}";
            return false;
        }
    }
    ...
    private static EditorSettings Load()
    {
        ...
        {
            // Corrupt settings fall back to defaults. Set up before the logger exists, so preserve the
