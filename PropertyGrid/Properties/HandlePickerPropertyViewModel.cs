...
public sealed class HandlePickerPropertyViewModel : PickerPropertyViewModel
{
    ...
    public HandlePickerPropertyViewModel(
        PropertyDescriptor descriptor, IValueAccessor accessor, AssetCatalog? catalog)
        : base(descriptor, accessor)
    {
        _catalog = catalog;
        _typeFilter = descriptor.Choices;
        if (catalog is not null)
            catalog.Changed += OnCatalogUpdated;
        RefreshDisplay();
    }

    public override Icon IconName => Icon.Target;
