...
public sealed class Clipboard
{
    ...
    /// with an optional <paramref name="name"/> carried back alongside the body.
    /// </summary>
    ...
    /// <summary>
    /// Writes plain text to the clipboard, outside the tagged JSON envelope — for ordinary text like a copied
    /// log selection. <see cref="PasteText"/> (or any other app) reads it back verbatim; the typed
    /// <see cref="Paste{T}"/> ignores it (it carries no kind tag), so plain text and typed payloads never
    /// masquerade as one another. The specialised, non-generic counterpart to <see cref="Copy{T}"/>.
    /// </summary>
    public Task CopyText(string text) =>
        Os is { } clipboard ? clipboard.SetTextAsync(text) : Task.CompletedTask;

    /// <summary>
    /// The clipboard's current plain text, or null when it is empty. Returns whatever text is there verbatim
    /// (including one of our own JSON envelopes) — it's the raw-text counterpart to <see cref="Paste{T}"/>,
    /// which instead only reads a matching typed payload.
    /// </summary>
    public async Task<string?> PasteText()
    {
