...
/// class-level [EngineSync] sets the family default (asset edits push through
/// <see cref="EngineCommands.AssetSet"/>) and declares the wire address every payload carries.
/// Identity and versioning are engine-owned (they live in the asset's <c>.meta</c>), so both mirror in
/// and never push.
///
...
/// </summary>
[EngineSync(EngineCommands.AssetSet, Address = "asset/{Id}")]
public abstract partial class Asset
{
    ...
    [EngineSync(EngineCommands.AssetSet, SyncMode.Mirror)]
    ...
    /// the next <see cref="SaveAsync"/>.</summary>
    public string Name
    {
        get => _name;
        set
    ...
    /// Empty while a blank mirror has no location.</summary>
    public string Path
    {
    ...
    /// <see cref="EngineCommands.AssetSet"/> as they were made. The save carries the current
    ...
    /// flip rides the edit's own change notification (this runs just before it).</summary>
    ...
    // The constructor-started load: binds (the address needs the id), then hydrates the whole body
    // from the engine's describe. A failed hydration unbinds — better an inert mirror on its defaults
    // than one silently presenting them as the asset's state.
