...
/// <summary>
/// Runs a data-driven toolbar command (an ordered list of RPC steps) against the engine. The generic RPC step
/// dispatch lives on <see cref="Engine.RunAsync"/>; this runner adds the editor glue — stepping the command,
/// routing <c>view.setGizmo</c> through <see cref="GizmoTool"/> and the
...
/// (Editor verbs that coordinate the selection/clipboard now live as behaviours on the
/// <see cref="Entity"/>/<see cref="World"/> objects, invoked directly by the context menus rather than as
/// command steps.)
/// </summary>
public sealed class ToolCommandRunner
{
    ...
    public ToolCommandRunner(Engine engine, GizmoTool gizmo, Session session, Logger log)
    {
        _engine = engine;
        _gizmo = gizmo;
        _session = session;
    ...
    private async Task<Result> RunRpcAsync(RpcCall? call, CancellationToken ct)
    {
        ...
        // A data-driven step is either a fire-and-forget notification (e.g. view.setGizmo) or an awaited
        // request whose failure stops the rest of the command.
        Result result;
        if (call.Notify)
        {
            await _engine.SendNotification(call.Method, call.Params).ContinueOnAnyContext();
            result = Result.Ok();
        }
        else
        {
            result = await _engine.SendCommand(call.Method, call.Params, ct).ContinueOnAnyContext();
        }
