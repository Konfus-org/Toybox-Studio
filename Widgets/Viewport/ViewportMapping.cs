...
/// <summary>
/// The Stretch=Uniform mapping shared by the surface display (<see cref="CompositionInteropViewport"/>) and
/// viewport picking: the engine's shared texture is scaled to fit the control while preserving its aspect
/// ratio (centred, with letterbox/pillarbox bars on the short axis), so a narrow or short panel never squishes
/// the image. Both the displayed image rect and the rect a click is normalized against come from
/// <see cref="ImageRect"/>, so what is shown and what is picked can never drift apart.
/// </summary>
internal static class ViewportMapping
{
    /// <summary>
    /// The destination rect (offset + size) the surface occupies inside the control: the largest centred rect
    /// that preserves the surface's aspect ratio. Falls back to the whole control when a size is degenerate.
    /// </summary>
        ...
        double controlW, double controlH, double surfW, double surfH)
    {
        ...
        var scale = Math.Min(controlW / surfW, controlH / surfH);
    ...
    /// pointer). Returns false when the point falls in the letterbox bars (outside the image) or the sizes are
    /// degenerate.
    /// </summary>
