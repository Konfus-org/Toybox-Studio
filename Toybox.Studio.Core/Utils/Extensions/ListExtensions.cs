...
{
    /// <summary>
    /// Reconciles <paramref name="target"/> to match <paramref name="desired"/> in place — removing, inserting,
    /// and moving only the items that actually changed — instead of Clear() + re-Add. A Clear raises a Reset that
    /// makes a bound <c>ItemsControl</c> drop and rebuild every container, which for a <c>TreeView</c> collapses
    /// expansion, resets scroll, and flashes the whole list. Reconciling in place keeps the unchanged containers,
    /// so adding or moving one entity touches only that row and feels smooth.
    ///
    ...
    /// </summary>
    public static void Reconcile<T>(this ObservableCollection<T> target, IReadOnlyList<T> desired)
    {
