...
/// </summary>
public sealed partial class WorldViewModel : ObservableObject, IWorldMenuTarget
{
    ...
    public WorldViewModel(
        ...
        WorldContextMenu worldMenu)
    {
        _world = world;
        _selection = selection;
        _components = components;
        _scripts = scripts;
        world.Active.Changed += roots => Dispatch.To(DispatchContext.UI, () => Reconcile(roots));
        // Any reflected edit dirties the world automatically; the view only needs the moment it happened, so the
        // live play-mode pull can defer to a value the user just changed (see RefreshSelectedEntityAsync).
        world.Active.Edited += () => _lastEditUtc = DateTime.UtcNow;
        selection.SelectionChanged +=
            () => Dispatch.To(DispatchContext.UI, () => ResolveSelection(_selection.SelectedId, reveal: true));
        // The "Rename" context-menu verb is a view-layer action (inline edit in the tree); the world menu
        // raises it and the world view performs it on the matching row.
        worldMenu.RenameRequested += id => Dispatch.To(DispatchContext.UI, () => BeginRenameEntity(id));
        // Move up/down is view-model logic: the menu decides visibility, the world view computes the sibling
        // reorder and applies it by setting the entity's Order.
        worldMenu.MoveRequested +=
            (id, delta) => Dispatch.To(DispatchContext.UI, () => MoveEntityOrderAsync(id, delta).FireAndForget());
    }

    // The tree's blank area is a world menu target: no entity, on the tree surface (add / paste-entity rows).
    ...
    {
        var entity = _world.Active.GetEntity(id);
        ...
        if (changeBucket)
        {
            ...
            if (!promote.Success)
            {
                ...
                await _world.Active.RefreshAsync().ContinueOnSameContext();
                return;
            }
        }
    ...
    /// <summary>
    /// Moves an entity one slot up (<paramref name="delta"/> -1) or down (+1) among its same-bucket siblings by
    /// setting its <see cref="Entity.Order"/> — the "move up/down" logic the entity menu delegates here. A no-op at
    /// the ends. Swaps order with the neighbour it passes, then re-syncs to engine truth.
    /// </summary>
    private async Task MoveEntityOrderAsync(ulong id, int delta)
    {
        var world = _world.Active;
        if (world.Find(id) is not { } entity)
            return;

        var siblings = SiblingsOf(entity);
        var index = siblings.FindIndex(sibling => sibling.Id == id);
        var target = index + delta;
        if (index < 0 || target < 0 || target >= siblings.Count)
            return;

        // Swap the explicit order with the neighbour so the two trade places; both are reflected pushes.
        var neighbour = siblings[target];
        (entity.Order, neighbour.Order) = (neighbour.Order, entity.Order);
        await world.RefreshAsync().ContinueOnSameContext();
    }

    /// <summary>The entity's display-bucket siblings (same parent + same global/streamed), ordered by
    /// <c>(Order, Id)</c> — the engine's sibling-renumber order. Shared by the menu's can-move check and the
    /// reorder above.</summary>
    internal static List<Entity> SiblingsOf(Entity entity)
    {
        var bucket = entity.ParentId == 0
            ? entity.World.Entities
            : entity.World.Find(entity.ParentId)?.Children ?? [];
        return bucket
            .Where(sibling => sibling.IsGlobal == entity.IsGlobal)
            .OrderBy(sibling => sibling.Order)
            .ThenBy(sibling => sibling.Id)
            .ToList();
    }
    ...
    {
        var result = await _world.Active.GetEntity(id).SetGlobalAsync(global, CancellationToken.None)
            .ContinueOnSameContext();
        if (!result.Success)
    ...
    public async Task RefreshSelectedEntityAsync()
    {
        ...
        try
        {
            var result = await _world.Active.GetEntity(entity.Id)
    ...
    [RelayCommand]
    private Task RefreshAsync() => _world.Active.RefreshAsync();
    ...
    {
        var result = await _world.Active.CreateEntityAsync("Entity", parent: 0UL, CancellationToken.None)
            .ContinueOnSameContext();
    ...
    private async Task DeleteEntityAsync(EntityViewModel? entity)
    {
        ...
        var result = await _world.Active.GetEntity(entity.Id).DestroyAsync(CancellationToken.None)
            .ContinueOnSameContext();
        if (!result.Success)
        {
    ...
    private async Task AddComponentAsync()
    {
        ...
        var result = await _world.Active.GetEntity(entity.Id).AddComponentAsync(pick.Key, CancellationToken.None)
            .ContinueOnSameContext();
        if (!result.Success)
        {
    ...
    private async Task AddScriptAsync()
    {
        ...
        var options = _scripts.Scripts
            .Select(asset => new CatalogItem(asset.Id.ToString(), asset.Name, "Script", Icon.ScrollText, Toybox.Studio.Utils.Colors.Green))
            .OrderBy(item => item.Title, StringComparer.OrdinalIgnoreCase)
            .ToList();
        ...
        var pick = await CatalogPicker
            ...
            .ContinueOnSameContext();
        if (pick is null || !ulong.TryParse(pick.Key, out var scriptId))
            return;

        var result = await _world.Active.GetEntity(entity.Id).AddScriptAsync(scriptId, CancellationToken.None)
            .ContinueOnSameContext();
        if (!result.Success)
        {
        ...
        List<EntityViewModel> globalRoots)
    {
        ...
        {
            entity = new EntityViewModel(data, () => _world.Active.RefreshAsync());
