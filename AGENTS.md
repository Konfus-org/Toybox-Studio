...
- `Ecs/` — the entity/component domain, built on the engine-sync codegen: `Entity.cs` (synced properties + generated structural commands), `Component.cs` (the family root whose class-level `[EngineSync]` sets the `component.set` default and roots the hierarchy), and the concrete components (`Transform.cs`, …). Domain objects raise `IListenable.Changed` only; the world tree and inspector bind to explicit view-models (to come with those panels).
