...
/// expanded flag (a section header's <c>IsChecked</c>, a row's <c>ToggleOnTapBehavior.State</c>, a tree node's
...
/// shared <see cref="PopBehavior"/> hover-grow rather than a <c>TranslateTransform.Y</c> sub-property: every nod-able
/// row also has PopBehavior, which keeps <c>RenderTransform</c> set to a <see cref="TransformOperations"/>, leaving a
/// sub-property animator with no transform to drive — so the nod must speak the same language to play at all.
/// The animation reverts (<see cref="FillMode.None"/>) to PopBehavior's value when it ends, so the row settles back to
...
public static class ExpandNodBehavior
{
    ...
    {
        // PopBehavior drives RenderTransform through a TransformOperationsTransition, but a keyframe Animation uses a
    ...
    private static void Nod(Control control, bool expanding)
    {
        ...
        var animation = new Animation
        {
            ...
            // Revert to the control's own RenderTransform (PopBehavior's hover/rest scale) when the nod finishes, so
    ...
    // PopBehavior), so the animator never has to reach into a sub-transform that the PopBehavior-set TransformOperations lacks.
