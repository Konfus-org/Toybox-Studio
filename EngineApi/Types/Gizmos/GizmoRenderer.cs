...
public sealed class GizmoRenderer
{
    ...
    /// <summary>A solid round cylinder of <paramref name="radius"/> between two points — the round-shaft
    /// counterpart of <see cref="SolidBeam"/>.</summary>
    public GizmoRenderer SolidCylinder(Vector3 from, Vector3 to, float radius) =>
        Add("solid_cylinder", WireValue.Write(from), WireValue.Write(to), WireValue.Write(radius));

    /// <summary>A solid round cone with a circular base of <paramref name="radius"/> at
    /// <paramref name="baseCenter"/> and apex at <paramref name="tip"/> — the translate arrowhead.</summary>
    public GizmoRenderer SolidCone(Vector3 baseCenter, Vector3 tip, float radius) =>
        Add("solid_cone", WireValue.Write(baseCenter), WireValue.Write(tip), WireValue.Write(radius));
