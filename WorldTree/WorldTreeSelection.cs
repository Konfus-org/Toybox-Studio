...
/// <summary>
/// Drives multi-selection across the world panel's selectors (the Globals and Streamed trees and the flat
...
///
/// Bind <c>local:WorldTreeSelection.Selection="{Binding Selection}"</c> on every selector that shares the
/// selection. Like <see cref="DragDrop"/>, it is entity-aware so it can resolve ids to the row view-models a
/// tree highlights, keeping the view free of code-behind routing.
/// </summary>
public static class WorldTreeSelection
{
    ...
    private static void OnSelectionChanged(Control control, AvaloniaPropertyChangedEventArgs args)
    {
        ...
        var group = Groups.GetValue(selection, static s =>
        {
            ...
            s.Changed += () => PushToControls(created);
            return created;
        });
    ...
    private static void OnUserSelectionChanged(Control control, Group group, SelectionChangedEventArgs e)
    {
        ...
        foreach (var added in e.AddedItems.OfType<WorldEntityViewModel>())
        {
            ids.Remove(added.Id);
            ids.Add(added.Id);
        }
    ...
    private static void BringIntoView(TreeView tree, ulong primary, IReadOnlyList<WorldEntityViewModel> items)
    {
    ...
    private static List<WorldEntityViewModel> ResolveItems(Control control, HashSet<ulong> wanted)
    {
        var matches = new List<WorldEntityViewModel>();
        ...
        if (control is TreeView)
            ...
            foreach (var item in roots.OfType<WorldEntityViewModel>())
    ...
    private static void CollectTree(
        IEnumerable roots, HashSet<ulong> wanted, List<WorldEntityViewModel> matches)
    {
        foreach (var item in roots.OfType<WorldEntityViewModel>())
        {
            if (wanted.Contains(item.Id))
    ...
    private static IEnumerable<ulong> SelectedIdsOf(Control control) =>
        (SelectedItemsOf(control)?.OfType<WorldEntityViewModel>() ?? [])
        .Select(item => item.Id)
        .ToList();
