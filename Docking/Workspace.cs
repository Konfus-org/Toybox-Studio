...
/// hooks fire (e.g. Settings' open/close lifecycle), and emptied floating windows are swept away. Building,
...
public sealed class Workspace
{
    ...
    internal Workspace(IReadOnlyList<DockableDescriptor> dockables, Popups popups, DockFactory factory)
    {
        ...
        // Dock also shows floats as unowned, non-topmost top-levels, so clicking back into the
        // (often maximized) main window buries them behind it — keep them above it as it opens.
        _factory.WindowOpened += (_, args) =>
        {
            FocusFloatContent(args.Window);
            KeepFloatAboveMain(args.Window);
        };
        ...
        // Track the focused panel's view-model, so "active document" actions (Undo/Redo) reach whichever
        // panel the user is in.
        _factory.FocusedDockableChanged += (_, args) => SetFocused(args.Dockable);
    }
    ...
    /// <summary>
    /// The view-model of the panel that currently has focus (Dock's focused dockable), or null — the
    /// "active document" that focus-directed actions target (Undo/Redo route here). Changes raise
    /// <see cref="FocusedChanged"/>.
    /// </summary>
    public object? Focused { get; private set; }

    /// <summary>Raised when <see cref="Focused"/> changes.</summary>
    ...
    /// <summary>The registered dockable behind a live tool id (a spawned "Key#N" matched by its base
    /// key), or null when the id maps to no current dockable — used by the layout restore to tell a
    /// transient on-demand panel from a seeded one.</summary>
    internal DockableDescriptor? DescriptorFor(string toolId) =>
        TryResolveDescriptor(toolId, out var descriptor) ? descriptor : null;

    // Resolves Dock's newly-focused dockable to its tracked view-model and publishes the change (a
    // non-tool, or a tool whose instance isn't tracked, focuses nothing — no active document).
    private void SetFocused(IDockable? dockable)
    {
        var viewModel = dockable is { Id: { } id } ? _instances.GetValueOrDefault(id) : null;
        if (ReferenceEquals(viewModel, Focused))
            return;
    ...
    // Idempotent across the repeated passes a restore / re-template triggers. Returns false when the
    // tool's id no longer maps to any registered dockable (a removed or renamed panel): the tool can't
    // be brought up, so it would otherwise render as a blank husk — the restore treats it as stale and
    // falls back to the default layout.
    internal bool AttachTool(Tool tool)
    {
    ...
    // Dock presents a float as an unowned, non-topmost top-level: Avalonia can't bind an owner after the
    // window is shown (Present calls the parameterless Show), and the OS raises the main window over an
    // unowned sibling when it's clicked — so a floated Asset Viewer / Coder vanishes behind the main
    // window the moment you interact with it. Topmost keeps the tool window visible above it so you can
    // browse and preview at once. (Settable post-show, unlike Owner — the one reliable seam here.)
    private static void KeepFloatAboveMain(IDockWindow? window)
    {
        if (window?.Host is Window host)
            host.Topmost = true;
    }
    ...
    {
        DockPanelRecord tool =
            typeof(ISplitHost).IsAssignableFrom(descriptor.ViewModelType) ? new SplitPanelRecord()
            : typeof(IToolbarHost).IsAssignableFrom(descriptor.ViewModelType) ? new ToolbarPanelRecord()
            : new DockPanelRecord();
        tool.Id = id;
    ...
    private void WireToolBindings(Tool tool, object viewModel)
    {
        ...
        // A split-hosting panel (the viewport grid) gets its persisted split tree — each pane's own
        // toolbar placements ride inside it, bound per pane by the host as it materializes them.
        if (tool is SplitPanelRecord split && viewModel is ISplitHost splitHost)
            splitHost.BindSplitLayout(split.Layout);
    ...
    private void DropInstance(string toolId)
    {
        ...
        // A closed panel can't be the active document any more.
        if (ReferenceEquals(viewModel, Focused))
        {
            Focused = null;
            FocusedChanged?.Invoke();
