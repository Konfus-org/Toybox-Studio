...
/// <see cref="ToolbarView"/> is overlaid on (the viewport's panel).
/// </summary>
public static class ToolbarDockDrag
{
    ...
    // The outer fraction of an edge that reads as its corner rather than its centre run.
    private const double CornerFraction = 0.25;

    // The placement the pointer chooses within the host bounds: the nearest edge, refined into a
    // corner when the pointer also sits in that edge's outer quarter (so every corner is reachable
    // from either of its edges).
    private static ToolbarEdge NearestEdge(Point pointer, Size host)
    {
        ...
        return RefineToCorner(edge, pointer, host);
    }

    private static ToolbarEdge RefineToCorner(ToolbarEdge edge, Point pointer, Size host)
    {
        switch (edge)
        {
            case ToolbarEdge.Top or ToolbarEdge.Bottom:
                if (pointer.X < host.Width * CornerFraction)
                    return edge == ToolbarEdge.Top ? ToolbarEdge.TopLeft : ToolbarEdge.BottomLeft;
                if (pointer.X > host.Width * (1 - CornerFraction))
                    return edge == ToolbarEdge.Top ? ToolbarEdge.TopRight : ToolbarEdge.BottomRight;
                return edge;

            case ToolbarEdge.Left or ToolbarEdge.Right:
                if (pointer.Y < host.Height * CornerFraction)
                    return edge == ToolbarEdge.Left ? ToolbarEdge.TopLeft : ToolbarEdge.TopRight;
                if (pointer.Y > host.Height * (1 - CornerFraction))
                    return edge == ToolbarEdge.Left ? ToolbarEdge.BottomLeft : ToolbarEdge.BottomRight;
                return edge;
