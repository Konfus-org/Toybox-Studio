...
{
    private readonly IReadOnlyList<AssetMeta> _options;

    public AssetPickerViewModel(string title, IReadOnlyList<AssetMeta> options, ulong currentId)
    {
        Title = title;
        ...
        _options = options.Where(o => o.Id != 0).Prepend(new AssetMeta(0, "None", "", "")).ToList();
    ...
    [ObservableProperty]
    public partial IReadOnlyList<AssetMeta> FilteredOptions { get; set; }

    [ObservableProperty]
    [NotifyCanExecuteChangedFor(nameof(SelectCommand))]
    public partial AssetMeta? SelectedAsset { get; set; }
