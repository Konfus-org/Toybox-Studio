...
public sealed class MenuBuilder
{
    ...
    // Builds the visible rows into the shared menu surface; null when nothing real is left to show. Each menu
    // stars under its own host (its name), so a row's label scopes to this menu alone.
    internal SearchableMenuViewModel? Compose(FavoritesManager favorites, EventDispatcher events, string host)
    {
        ...
        var menu = new SearchableMenuViewModel(
            host,
            close => items
                .Select(item => new MenuEntryViewModel(item, host, favorites, close))
                .ToList(),
            events);
