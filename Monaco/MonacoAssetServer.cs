...
/// <summary>
/// Serves the vendored Monaco bundle (copied to the output under <c>Monaco</c>) over a loopback HTTP server so
/// the embedded WebView can load it from a real origin. We go through HTTP rather than <c>file://</c> or
/// WebView2 virtual-host mapping because Monaco spins up web workers, which need a trustworthy origin — and
/// <c>http://localhost</c> is treated as a secure context by Chromium, so workers, <c>crypto.subtle</c>, etc.
/// all work without bundling a backend-specific COM call. Loopback-only and read-only (GET/HEAD), it mirrors
/// how Studio already talks to the engine over loopback TCP. One instance is shared by every editor surface.
/// </summary>
public sealed class MonacoAssetServer : IDisposable
{
    ...
    {
        _log = log;
        _root = Path.Combine(AppContext.BaseDirectory, "Monaco");
    }
    ...
    public Result<Uri> EnsureStarted()
    {
        lock (_gate)
        {
            ...
            _log.Info($"Code editor assets served at {prefix}");
            return Result<Uri>.Ok(_baseUri);
        }
    }
