...
/// data so users can eventually author their own; persisted as part of the dock layout (the icon persists as its
/// Lucide name via the converter; the colour as its <see cref="PaletteColor"/> token name, resolved via
/// <c>Colors.ToColor</c> for display).
/// </summary>
public sealed class ToolbarItem
{
    ...
    /// <summary>Lucide icon (see <c>IconView</c>).</summary>
    [JsonConverter(typeof(IconJsonConverter))]
    public Icon Icon { get; set; }

    /// <summary>Optional icon colour as a <see cref="PaletteColor"/> token; <see cref="PaletteColor.None"/> =
    /// themed default.</summary>
    [JsonConverter(typeof(StringEnumConverter))]
    public PaletteColor IconColor { get; set; }
