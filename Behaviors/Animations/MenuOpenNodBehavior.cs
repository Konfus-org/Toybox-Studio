...
/// side. Mirrors <see cref="EnterNodBehavior"/> (a dip-and-return on a Visual's <c>TranslateTransform</c>) but on the
...
public static class MenuOpenNodBehavior
{
    ...
    // the EnterNodBehavior feel. The keyframes return to 0, so the panel is left exactly where layout placed it.
