...
/// <see cref="DockableDescriptor"/>s, with each view-model built on open by the <see cref="ViewModelFactory"/>
/// — its services resolved from the container automatically, so panels are never service-registered and
/// nothing hand-authors a per-panel factory. Parent references are resolved and validated the same way: an
/// unknown parent or a parent cycle logs a warning and the dockable falls back to docking against the main
/// window.
/// </summary>
public sealed class DockableCatalog
{
    public DockableCatalog(ViewModelFactory viewModels, Logger log, params Assembly[] assemblies)
    {
        var entries = Scan(assemblies).ToList();
        Dockables = entries
            .Select(entry => Build(entry, viewModels, log))
    ...
    private static DockableDescriptor Build(
        (Type View, DockableAttribute Attribute, Type ViewModel) entry, ViewModelFactory viewModels, Logger log)
    {
        ...
        return new DockableDescriptor
        {
            ...
            CreateViewModel = () => viewModels.Create(viewModelType),
