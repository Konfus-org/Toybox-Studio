...
public sealed partial class ScriptContainerViewModel : ObservableObject
{
    ...
    public ScriptContainerViewModel(
        Component component, Func<Task> resync)
    {
        ...
        Bindings = [];
        var properties = JsonDescriptorReader.Read(_raw);
        var scripts = properties.FirstOrDefault(property => property.Descriptor.Name == ScriptsProperty);
        if (scripts.Descriptor is { } scriptsDescriptor)
        {
            for (var index = 0; index < scriptsDescriptor.Children.Count; index++)
            {
                var bindingDescriptor = scriptsDescriptor.Children[index];
                var bindingAccessor = scripts.Accessor.Element(index);
    ...
    private static JToken? Inner(JToken? token) => token.Unwrap();
